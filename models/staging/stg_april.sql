...
with source as (
    select * from {{ source('staging_medicines', 'April') }}
),
...
renamed as (
    select
        ...
        '01 - april' as month,
        cast(nullif(trim("Remaining_Qty"), '') as numeric) as initial_starting_quantity,
        cast(nullif(trim("Tablet_Old_Stock"), '') as numeric) as monthly_starting_quantity,
        cast(nullif(trim("Tablet_Old_Stock"), '') as numeric) - cast(nullif(trim("Tablet_Stock_Out_1"), '') as numeric) as monthly_ending_quantity,
